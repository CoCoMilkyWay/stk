      ...
      for (size_t i = 0; i < nb; ++i) {
        ...
        for (size_t k = 0; k < n_draw; ++k) {
          ...
          for (size_t t = 0; t < VR; ++t) {
            ...
            rt.day_buf[t] = ok ? x : kNaN; // 缺/门控/±inf 记 NaN, 谱前 ffill_day (不填 0, 见其注释)
            n_ok += ok;
            if (ok)
          ...
          if (n_ok > 0) {
            ffill_day(rt.day_buf.data(), VR);
            [[maybe_unused]] const bool done = rt.psd.compute(rt.day_buf.data());
            assert(done && "ffill 后稠密, 有效数 = VR");
            double total = 0.0;
            for (size_t q = 1; q <= kPvPsdPts; ++q)
              total += static_cast<double>(rt.psd.power[q]);
            if (total > 0.0) {
              const double inv = 1.0 / total;
              auto &sum = rt.psd_sum[slot];
              for (size_t q = 1; q <= kPvPsdPts; ++q)
                sum[q] += static_cast<double>(rt.psd.power[q]) * inv;
            }
            ++rt.psd_n[slot];
          }
        }
