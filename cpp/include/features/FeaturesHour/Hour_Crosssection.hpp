...
inline void compute_cs_hour(GlobalFeatureStore *store,
  ...
  // Build valid indices
  const _Float16 *valid_flags = CS_READ_ALL(store, date, level_idx, t_hour, L2_FieldOffset::universe_size);
  valid_indices.clear();
  ...
  {
    const _Float16 *input = CS_READ_ALL(store, date, level_idx, t_hour, L2_FieldOffset::hour_ret_12h_mom);
    for (size_t a = 0; a < A; ++a)
    ...
    CS_WRITE_ALL(store, date, level_idx, t_hour, L2_FieldOffset::cs_hour_return_beta, output_fp16.data(), A);
  }

  // CS feature 2: cs_hour_liq_adj_ret
  {
    const _Float16 *input = CS_READ_ALL(store, date, level_idx, t_hour, L2_FieldOffset::hour_volatility);
    for (size_t a = 0; a < A; ++a)
      input_fp32[a] = input[a];
    std::fill(output_fp32.begin(), output_fp32.end(), 0.0f);
    compute_rank_inverse_normal_sparse(input_fp32.data(), valid_indices, output_fp32.data());
    for (size_t a = 0; a < A; ++a)
      output_fp16[a] = output_fp32[a];
    CS_WRITE_ALL(store, date, level_idx, t_hour, L2_FieldOffset::cs_hour_liq_adj_ret, output_fp16.data(), A);
  }

  // CS feature 3: cs_hour_range_rank
  {
    const _Float16 *input = CS_READ_ALL(store, date, level_idx, t_hour, L2_FieldOffset::pivot_dev);
    for (size_t a = 0; a < A; ++a)
      input_fp32[a] = input[a];
    std::fill(output_fp32.begin(), output_fp32.end(), 0.0f);
    compute_rank_inverse_normal_sparse(input_fp32.data(), valid_indices, output_fp32.data());
    for (size_t a = 0; a < A; ++a)
      output_fp16[a] = output_fp32[a];
    CS_WRITE_ALL(store, date, level_idx, t_hour, L2_FieldOffset::cs_hour_range_rank, output_fp16.data(), A);
  }
