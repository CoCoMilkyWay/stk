...
inline void compute_cs_minute(GlobalFeatureStore *store,
  ...
  // Build valid indices
  const _Float16 *valid_flags = CS_READ_ALL(store, date, 1, t_minute, L1_FieldOffset::universe_size);
  valid_indices.clear();
  ...
  {
    const _Float16 *input = CS_READ_ALL(store, date, 1, t_minute, L1_FieldOffset::min_ret_z);
    for (size_t a = 0; a < A; ++a)
    ...
    CS_WRITE_ALL(store, date, 1, t_minute, L1_FieldOffset::cs_min_return_rank, output_fp16.data(), A);
  }

  // CS feature 2: cs_min_volume_pct
  {
    const _Float16 *input = CS_READ_ALL(store, date, 1, t_minute, L1_FieldOffset::momentum_15m);
    for (size_t a = 0; a < A; ++a)
      input_fp32[a] = input[a];
    std::fill(output_fp32.begin(), output_fp32.end(), 0.0f);
    compute_rank_inverse_normal_sparse(input_fp32.data(), valid_indices, output_fp32.data());
    for (size_t a = 0; a < A; ++a)
      output_fp16[a] = output_fp32[a];
    CS_WRITE_ALL(store, date, 1, t_minute, L1_FieldOffset::cs_min_volume_pct, output_fp16.data(), A);
  }

  // CS feature 3: cs_min_spread_z
  {
    const _Float16 *input = CS_READ_ALL(store, date, 1, t_minute, L1_FieldOffset::rv_5m_norm);
    for (size_t a = 0; a < A; ++a)
      input_fp32[a] = input[a];
    std::fill(output_fp32.begin(), output_fp32.end(), 0.0f);
    ...
    for (size_t a = 0; a < A; ++a)
      output_fp16[a] = output_fp32[a];
    CS_WRITE_ALL(store, date, 1, t_minute, L1_FieldOffset::cs_min_spread_z, output_fp16.data(), A);
  }
