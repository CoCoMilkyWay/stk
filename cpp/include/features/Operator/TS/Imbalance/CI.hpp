...
// #define FIELDS_L0_Ci_1(X, CAT1) X(obi_1, CAT1, RATIO, NONE, "Order Book Imbalance 1-Level", "顶部1档失衡", "顶部1档订单失衡率", R"(\frac{V_{1,t}^{M,B} - V_{1,t}^{M,A}}{V_{1,t}^{M,B} + V_{1,t}^{M,A}})", OP(Ci_1))

#define NODE_Ci_5(N) N(Ci_5, (CI<5>), (Depth.bid_qty, Depth.ask_qty), onMinute)
#define FIELDS_L1_Ci_5(X, CAT1) X(obi_5, CAT1, RATIO, NONE, "Order Book Imbalance 5-Level", "累计5档失衡", "累计5档订单失衡率(降频)", R"(\frac{\sum_{i=1}^{5}(V_{i,t}^{M,B} - V_{i,t}^{M,A})}{\sum_{i=1}^{5}(V_{i,t}^{M,B} + V_{i,t}^{M,A})})", OP(Ci_5))

#define NODE_Ci_10(N) N(Ci_10, (CI<10>), (Depth.bid_qty, Depth.ask_qty), onMinute)
#define FIELDS_L1_Ci_10(X, CAT1) X(obi_10, CAT1, RATIO, NONE, "Order Book Imbalance 10-Level", "累计10档失衡", "累计10档订单失衡率(降频)", R"(\frac{\sum_{i=1}^{10}(V_{i,t}^{M,B} - V_{i,t}^{M,A})}{\sum_{i=1}^{10}(V_{i,t}^{M,B} + V_{i,t}^{M,A})})", OP(Ci_10))

#define NODE_Ci_30(N) N(Ci_30, (CI<30>), (Depth.bid_qty, Depth.ask_qty), onMinute)
#define FIELDS_L1_Ci_30(X, CAT1) X(obi_30, CAT1, RATIO, NONE, "Order Book Imbalance 30-Level", "累计30档失衡", "累计30档订单失衡率(降频)", R"(\frac{\sum_{i=1}^{30}(V_{i,t}^{M,B} - V_{i,t}^{M,A})}{\sum_{i=1}^{30}(V_{i,t}^{M,B} + V_{i,t}^{M,A})})", OP(Ci_30))
