...
// =============================================================================
//
// 功能: 计算 "吃单做多/做空 a分钟 b万元" 收益标签
//
// 时间线 (以5分钟为例, DELAY=3s):
//   label_l0          entry_l0              exit_l0
//      |--- 3s delay --->|--- 5min hold --->|
//      ^                 ^                  ^
//      标签时刻          建仓时刻            平仓时刻
//
// 冲击成本: forloop对手盘深度，直到b万元成交完毕，得到VWAP
//   - 做多建仓: 吃ask盘 (taker-buy)
//   - 做空建仓: 吃bid盘 (taker-sell)
//
// 费用:
//   - 买入: 万1佣金 (FEE_BUY = 0.0001)
//   - 卖出: 万11 (万10印花 + 万1佣金, FEE_SELL = 0.0011)
//
// 收益公式:
...
//
// 输出: 按hold_minutes分组 (便于TS_WRITE_FEATURES批量写入)
//   组0 (5min):  lb_long_5m_5w, lb_long_5m_20w, lb_short_5m_5w, lb_short_5m_20w
//   组1 (10min): lb_long_10m_5w, lb_long_10m_20w, lb_short_10m_5w, lb_short_10m_20w
//   组2 (30min): lb_long_30m_5w, lb_long_30m_20w, lb_short_30m_5w, lb_short_30m_20w
//
// 优化:
//   - 深度快照预计算: 每秒保存一次各金额的VWAP，避免重复计算
//   - 环形缓冲区: 只保留 max_delay+64 秒的快照
//   - 深度容错: 如果某秒无深度更新，向前查找最近60秒内的有效快照
//
// =============================================================================
...
public:
  ...
  static constexpr size_t MAX_HOLD = []() constexpr {
    ...
    for (size_t i = 0; i < HOLD_COUNT; ++i)
      if (HOLD_MINUTES[i] > m) m = HOLD_MINUTES[i];
    return m;
  }();
  ...
  inline void compute(size_t t) {
    ...
    // 重置输出
    for (size_t i = 0; i < LABEL_COUNT; ++i) out_valid_[i] = false;
    for (size_t h = 0; h < HOLD_COUNT; ++h) out_group_valid_[h] = false;
    ...
    for (size_t h = 0; h < HOLD_COUNT; ++h) {
      ...
      if (t < total_delay) continue;  // 数据不足
  ...
  // 每日重置
  inline void reset() {
    for (auto &snap : buffer_) snap.valid = false;
  }
...
private:
  ...
  inline float calc_return(const Snapshot *entry, const Snapshot *exit, size_t amt_idx, bool is_long) {
    if (is_long) {
      ...
      if (entry_vwap < 1e-6f || shares < 1e-6f) return 0.0f;
      ...
      if (exit_vwap < 1e-6f) return 0.0f;
  ...
  static inline float interp_vwap(const float *vwaps, const float *shares, float target) {
    if (AMT_COUNT == 1 || target <= shares[0]) return vwaps[0];
