...
//   fmeta        _meta 编码/解码唯一事实源 → features/MetaFlag.hpp (稳定头, 消费端不依赖本算子文件)
...
#define FIELDS_L0_Meta(X, CAT1) \
  X(_meta, CAT1, AUTO, "Meta Flags", "元数据", "0=无事件; ±micro price, 负=该秒盘口有更新", R"(\pm P_t^{micro} \cdot \mathbf{1}_{\mathrm{data}})", FLAG)

#define FIELDS_L1_Meta(X, CAT1) \
  X(_meta, CAT1, AUTO, "Meta Flags", "元数据", "0=无效分钟; ±1, 负=该分钟盘口有更新", R"(\pm\mathbf{1}_{\mathrm{data}})", FLAG)
