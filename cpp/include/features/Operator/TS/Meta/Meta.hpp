...
#define FIELDS_L0_Meta(X, CAT1)                                                                                                  \
  X(_depth_valid, CAT1, RAW, NONE, "Depth Valid Flag", "深度有效标志", "该秒盘口有更新", R"(\mathbf{1}_{\mathrm{depth}})", FLAG) \
  X(_data_valid, CAT1, RAW, NONE, "Data Valid Flag", "数据有效标志", "该秒有事件写入", R"(\mathbf{1}_{\mathrm{data}})", FLAG)

#define FIELDS_L1_Meta(X, CAT1) \
  X(_data_valid, CAT1, RAW, NONE, "Data Valid Flag", "数据有效标志", "该分钟有效(有成交)", R"(\mathbf{1}_{\mathrm{data}})", FLAG)
...
#define FIELDS_DEPTH_Meta(X, CAT1)                                                                                             \
  ...
  X(_depth_valid, CAT1, RAW, NONE, "Depth Valid Flag", "深度有效", "该分钟盘口有更新", R"(\mathbf{1}_{\mathrm{depth}})", FLAG) \
  X(_data_valid, CAT1, RAW, NONE, "Data Valid Flag", "数据有效", "该分钟有事件写入", R"(\mathbf{1}_{\mathrm{data}})", FLAG)
