...
// =============================================================================
//   agg_bid/ask = avg(log(P_order / P_best))   (买/卖单侵略性)
//   agg_diff = agg_bid - agg_ask                (侵略性差)
//   cpr = |O^C| / |O^M|                         (撤挂比)
//   agg_trend = linear_slope(agg_diff, 20s 窗口)   (侵略性趋势)
...
private:
  ...
  inline void flush_second_() {
    // 1. 平均侵略性
    y[agg_bid] = cnt_agg_buy_ > 0 ? sum_agg_buy_ / cnt_agg_buy_ : y[agg_bid];
    y[agg_ask] = cnt_agg_sell_ > 0 ? sum_agg_sell_ / cnt_agg_sell_ : y[agg_ask];

    // 2. 侵略性差: 正值买方更激进
    y[agg_diff] = y[agg_bid] - y[agg_ask];
    ...
    // 5. 侵略性趋势: agg_diff 滑动窗口的线性回归斜率
    agg_window_[agg_idx_] = y[agg_diff];
    ...
    if (agg_cnt_ >= 2) {
      ...
      y[agg_trend] = denom > 1e-6f ? (n * sum_xy - sum_x * sum_y) / denom : 0.0f;
    }
...
#define FIELDS_L1_Behav(X, CAT1)                                                                                                                                                                                                                                                                                                             \
  X(agg_bid, CAT1, RAW, "Bid Aggressiveness", "买单平均侵略性", "限价买单相对best bid的激进程度(降频)", R"(\frac{1}{\#O_W^{M,B,\mathrm{lmt}}}\sum_{i\in O_W^{M,B,\mathrm{lmt}}}\log\frac{P_i}{P_{1,\tau_i}^{M,B}}, \quad O_W^{M,B,\mathrm{lmt}}=\{i: \tau_i\in W, s_i=B, \mathrm{type}_i=\mathrm{limit}\})", OP(Behav, agg_bid, None, None)) \
  X(agg_ask, CAT1, RAW, "Ask Aggressiveness", "卖单平均侵略性", "限价卖单相对best ask的激进程度(降频)", R"(\frac{1}{\#O_W^{M,A,\mathrm{lmt}}}\sum_{i\in O_W^{M,A,\mathrm{lmt}}}\log\frac{P_{1,\tau_i}^{M,A}}{P_i}, \quad O_W^{M,A,\mathrm{lmt}}=\{i: \tau_i\in W, s_i=A, \mathrm{type}_i=\mathrm{limit}\})", OP(Behav, agg_ask, None, None)) \
  X(agg_diff, CAT1, RAW, "Aggressiveness Diff", "侵略性差", "买卖侵略性差值(降频)", R"(\bar{a}_W^{B} - \bar{a}_W^{A}, \quad \bar{a}_W^{s}=\frac{1}{\#O_W^{M,s,\mathrm{lmt}}}\sum_{i\in O_W^{M,s,\mathrm{lmt}}}\log\frac{P_i}{P_{1,\tau_i}^{M,s}}, \quad s \in \{B,A\})", OP(Behav, agg_diff, None, None))                                    \
  X(cpr, CAT1, RATIO, "Cancel-to-Post Ratio", "撤挂比", "撤单量占挂单量比例(降频)", R"(\frac{\sum_{\tau\in W}(|O_{\tau}^{C,B}|+|O_{\tau}^{C,A}|)}{\sum_{\tau\in W}(|O_{\tau}^{M,B}|+|O_{\tau}^{M,A}|)})", OP(Behav, cpr, None, None))                                                                                                        \
  X(agg_trend, CAT1, RAW, "Aggressiveness Trend", "侵略性趋势", "子窗口侵略性序列线性回归斜率(降频)", R"(\hat{\beta}_1, \quad \bar{a}_{\tau}=\hat{\beta}_0+\hat{\beta}_1\tau+\epsilon_{\tau}, \quad \tau\in\{t-W,\ldots,t\})", OP(Behav, agg_trend, None, None))                                                                             \
  X(ord_size, CAT1, RAW, "Avg Order Size", "平均单笔规模", "窗口内订单平均量(降频)", R"(\frac{1}{\#O_W^{M}}\sum_{i\in O_W^{M}}|O_i|)", OP(Behav, ord_size, None, None))
