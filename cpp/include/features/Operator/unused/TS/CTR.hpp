...
#define FIELDS_L1_Ctr(X, CAT1)                                                                                                                                                                                                                                                                                    \
  ...
  X(ctr_xl, CAT1, RATIO, "Cumulative XL Trade Ratio", "特大单累计成交占比", "从开盘到t时刻,特大单成交额占总成交额比例(降频)", R"(\frac{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B,\mathrm{XL}}|+|O_{\tau}^{T,A,\mathrm{XL}}|)}{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B}|+|O_{\tau}^{T,A}|)})", OP(Ctr, ctr_xl, None, None))    \
  X(ctr_l, CAT1, RATIO, "Cumulative L Trade Ratio", "大单累计成交占比", "从开盘到t时刻,大单成交额占总成交额比例(降频)", R"(\frac{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B,\mathrm{L}}|+|O_{\tau}^{T,A,\mathrm{L}}|)}{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B}|+|O_{\tau}^{T,A}|)})", OP(Ctr, ctr_l, None, None))             \
  X(ctr_m, CAT1, RATIO, "Cumulative M Trade Ratio", "中单累计成交占比", "从开盘到t时刻,中单成交额占总成交额比例(降频)", R"(\frac{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B,\mathrm{M}}|+|O_{\tau}^{T,A,\mathrm{M}}|)}{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B}|+|O_{\tau}^{T,A}|)})", OP(Ctr, ctr_m, None, None))             \
  X(ctr_s, CAT1, RATIO, "Cumulative S Trade Ratio", "小单累计成交占比", "从开盘到t时刻,小单成交额占总成交额比例(降频)", R"(\frac{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B,\mathrm{S}}|+|O_{\tau}^{T,A,\mathrm{S}}|)}{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B}|+|O_{\tau}^{T,A}|)})", OP(Ctr, ctr_s, None, None))             \
  X(cnbi, CAT1, RATIO, "Cumulative Net Buy Ratio", "累计净买入比率", "从开盘到t,主动买入成交额减主动卖出成交额(降频)", R"(\frac{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B}|-|O_{\tau}^{T,A}|)}{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B}|+|O_{\tau}^{T,A}|)})", OP(Ctr, cnbi, None, None))                                     \
  X(cnbi_xl, CAT1, RATIO, "Cumulative Net Buy Ratio XL", "特大单累计净买入比率", "特大单对累计净买入失衡的贡献(降频)", R"(\frac{N_t^{\mathrm{XL}}}{\sum_{c}|N_t^{c}|}, \quad N_t^{c}=\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B,c}|-|O_{\tau}^{T,A,c}|), \quad c\in\{\mathrm{XL,L,M,S}\})", OP(Ctr, cnbi_xl, None, None)) \
