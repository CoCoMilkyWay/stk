...
#define FIELDS_L1_Peak_ask(X, CAT1)                                                                                                                                              \
  X(peak_loc_ask, CAT1, RAW, "Ask Peak Location", "卖侧峰位置", "卖侧深度最大值所在档位(前5档)(降频)", R"(\arg\max_{i \in [1,5]} V_{i,t}^{M,A})", OP(Peak_ask, loc, None, None)) \
  X(peak_ratio_ask, CAT1, RATIO, "Ask Peak Concentration", "卖侧峰集中度", "卖侧最大档位深度/平均档位深度(前5档)(降频)", R"(\frac{\max_{i \in [1,5]} V_{i,t}^{M,A}}{\frac{1}{5}\sum_{i=1}^{5} V_{i,t}^{M,A}})", OP(Peak_ask, ratio, None, None))

#define NODE_Peak_bid(N) N(Peak_bid, (Peak<true>), (DepthData.bid_qty), onMinute)

#define FIELDS_L1_Peak_bid(X, CAT1)                                                                                                                                              \
  X(peak_loc_bid, CAT1, RAW, "Bid Peak Location", "买侧峰位置", "买侧深度最大值所在档位(前5档)(降频)", R"(\arg\max_{i \in [1,5]} V_{i,t}^{M,B})", OP(Peak_bid, loc, None, None)) \
  X(peak_ratio_bid, CAT1, RATIO, "Bid Peak Concentration", "买侧峰集中度", "买侧最大档位深度/平均档位深度(前5档)(降频)", R"(\frac{\max_{i \in [1,5]} V_{i,t}^{M,B}}{\frac{1}{5}\sum_{i=1}^{5} V_{i,t}^{M,B}})", OP(Peak_bid, ratio, None, None))
