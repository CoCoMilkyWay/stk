...
#define LEVEL_0_FIELDS(X)\
  ...
  X(voi1,               1,             DATA,  TS,   IMBALANCE,      RAW,        LOG_ZSCORE,  "Vol Order Imba 1-Level",       "订单失衡1档",    R"(\Delta V^B_1 - \Delta V^A_1)",                                       "对近端大单挂单(容易成交)非常敏感")\
  X(voi30,              1,             DATA,  TS,   IMBALANCE,      RAW,        LOG_ZSCORE,  "Vol Order Imba 30-Level",      "订单失衡30档",   R"(\sum_{i=1}^{30} w_i (\Delta V^B_i - \Delta V^A_i))",                 "对近端大单挂单(容易成交)非常敏感, 带深度线性衰减加权")\
  X(oir5,               1,             DATA,  TS,   IMBALANCE,      RATIO,      NONE,        "Order Imba Ratio 5-Level",     "失衡率5档",      R"(\frac{V^B - V^A}{V^B + V^A})",                                         "订单失衡率[-1,1]标准化")\
  X(oir10,              1,             DATA,  TS,   IMBALANCE,      RATIO,      NONE,        "Order Imba Ratio 10-Level",    "失衡率10档",     R"(\frac{\sum_{i=1}^{10}(V^B_i - V^A_i)}{\sum_{i=1}^{10}(V^B_i + V^A_i)})", "深度失衡率(研报:优于VOI)")\
  X(soir5,              1,             DATA,  TS,   IMBALANCE,      RATIO,      NONE,        "SOIR 5-Level Weighted",        "逐档失衡5档",    R"(\frac{\sum w_i \cdot \mathrm{SOIR}_i}{\sum w_i})",                   "逐档订单失衡率加权")\
  X(soir5s,             1,             DATA,  TS,   IMBALANCE,      RATIO,      NONE,        "SOIR Level-5 Single",          "第5档失衡",      R"(\mathrm{SOIR}_5)",                                                       "研报:单档效果优于加权")\
  X(soir10s,            1,             DATA,  TS,   IMBALANCE,      RATIO,      NONE,        "SOIR Level-10 Single",         "第10档失衡",     R"(\mathrm{SOIR}_{10})",                                                    "深层订单簿失衡")\
  X(soir30s,            1,             DATA,  TS,   IMBALANCE,      RATIO,      NONE,        "SOIR Level-30 Single",         "第30档失衡",     R"(\mathrm{SOIR}_{30})",                                                    "最深层订单簿信号")\
  /* ======== 价格类因子 (Price-based) ======== */\
  X(mpb,                1,             DATA,  TS,   MICROSTRUCTURE, DEVIATION,  NONE,        "Mid-Price Basis",              "市价偏离度",     R"(P_{\mathrm{trade}} - P_{\mathrm{mid}})",                                  "成交均价与中间价偏离(研报最佳)")\
  X(mpc1,               1,             DATA,  TS,   MOMENTUM,       RAW,        NONE,        "MPC Lag-1",                    "中间价变化1",    R"(\frac{M_t - M_{t-1}}{M_{t-1}})",                                       "中间价短期变化率")\
  X(mpc5,               1,             DATA,  TS,   MOMENTUM,       RAW,        NONE,        "MPC Lag-5",                    "中间价变化5",    R"(\frac{M_t - M_{t-5}}{M_{t-5}})",                                       "中间价中期变化率")\
  X(mpc5_max,           1,             DATA,  TS,   MOMENTUM,       RAW,        NONE,        "MPC5 Daily Max",               "MPC5日最大",     R"(\max_d(\mathrm{MPC5}_d))",                                              "日内MPC5极值(IC -9.39%)")\
  X(mpc5_skew,          1,             DATA,  TS,   MOMENTUM,       RAW,        NONE,        "MPC5 Daily Skew",              "MPC5日偏度",     R"(\mathrm{skew}(\mathrm{MPC5}_d))",                                         "日内MPC5偏度(夏普3.07)")\
  /* ======== 截面因子 (Cross-sectional) ======== */\
  X(cs_spread_rank,     1,             DATA,  CS,   LIQUIDITY,      RANK,       RANK_ZSCORE, "CS Spread Rank",               "价差截面排名",   R"(\Phi^{-1}(\mathrm{pctl}(\mathrm{spread})))",                             "spread截面rank→inverse normal")\
  X(cs_tobi_rank,       1,             DATA,  CS,   IMBALANCE,      RANK,       RANK_ZSCORE, "CS TOBI Rank",                 "失衡截面排名",   R"(\Phi^{-1}(\mathrm{pctl}(\mathrm{tobi})))",                               "tobi截面rank→inverse normal")\
  X(cs_liquidity_ratio, 1,             DATA,  CS,   LIQUIDITY,      RATIO,      ZSCORE,      "CS Liquidity Ratio",           "流动性比率截面", R"(\frac{\mathrm{top\_size}/\mathrm{median}}{z})",                         "top-of-book size截面z-score")\
  /* ======== 标签 (Labels) ======== */\
  X(next_tick_ret,      1,             DATA,  LB,   LABEL,          FUTURE_RET, NONE,        "Next Tick Return",             "下tick收益",     R"(\log\frac{\mathrm{mid}_{t+1}}{\mathrm{mid}_t})",                        "下一tick对数收益")\
  X(next_5tick_ret,     1,             DATA,  LB,   LABEL,          FUTURE_RET, NONE,        "Next 5-Tick Return",           "未来5tick收益",  R"(\log\frac{\mathrm{mid}_{t+5}}{\mathrm{mid}_t})",                        "未来5tick累计对数收益")\
  /* ======== 元数据 (Metadata) ======== */\
  X(universe_size,      1,             DATA,  SH,   META,           UNIVERSE,   NONE,        "Universe Size",                "全域规模",       R"(\#(\mathrm{valid}))",                                                   "当前有效合约数量")\
  X(_link_to_L1,        1,             ALL,   META, META,           RAW,        NONE,        "Link to L1",                   "L1时间索引",     R"(\mathrm{idx}_{L1})",                                                     "L0→L1时间映射")\
  X(_link_to_L2,        1,             ALL,   META, META,           RAW,        NONE,        "Link to L2",                   "L2时间索引",     R"(\mathrm{idx}_{L2})",                                                     "L0→L2时间映射")\
  X(_depth_valid,       1,             ALL,   META, META,           RAW,        NONE,        "Depth Valid Flag",             "深度有效标志",   R"(\mathbf{1}_{\mathrm{valid}})",                                          "LOB深度缓冲区完整性标记")\
  X(_data_valid,        1,             ALL,   META, META,           RAW,        NONE,        "Data Valid Flag",              "数据有效标志",   R"(\mathbf{1}_{\mathrm{valid}})",                                          "事件驱动稀疏性标记")\
...
#define LEVEL_1_FIELDS(X)\
  X(min_ret_z,          1, DATA, TS,   MOMENTUM,   NORMALIZED, WINSOR,      "Minute Return Z-score",       "分钟收益",       R"(\frac{r - \mu}{\sigma})",                          "分钟对数收益标准化")\
  X(rv_5m_norm,         1, DATA, TS,   VOLATILITY, NORMALIZED, LOG_ZSCORE,  "Realized Vol 5m",             "5分钟波动率",    R"(\log(\sigma_{5m}))",                                "5分钟波动率标准化")\
  X(vwap_gap_pct,       1, DATA, TS,   PRICE,      DEVIATION,  ZSCORE,      "VWAP Gap Percent",            "VWAP偏离",       R"(\frac{c - \mathrm{vwap}}{\mathrm{vwap}})",             "价格相对VWAP偏离")\
  X(momentum_15m,       1, DATA, TS,   MOMENTUM,   OSCILLATOR, ZSCORE,      "Momentum 15m",                "15分钟动量",     R"(\frac{\sum r}{\sigma})",                           "15分钟累计动量标准化")\
  X(range_squeeze,      1, DATA, TS,   VOLATILITY, RATIO,      CLIP,        "Range Squeeze",               "Range收窄",      R"(\frac{H - L}{\sigma})",                             "盘面窄幅程度")\
  X(cs_min_return_rank, 1, DATA, CS,   MOMENTUM,   RANK,       RANK_ZSCORE, "CS Minute Return Rank",       "分钟收益截面",   R"(\Phi^{-1}(\mathrm{pctl}(r)))",                        "分钟收益截面rank")\
  X(cs_min_volume_pct,  1, DATA, CS,   VOLUME,     RANK,       RANK_ZSCORE, "CS Minute Volume Percentile", "分钟量能百分位", R"(\mathrm{pctl}(\log(\mathrm{vol})))",                   "分钟volume截面排名")\
  X(cs_min_spread_z,    1, DATA, CS,   LIQUIDITY,  NORMALIZED, ZSCORE,      "CS Minute Spread Z-score",    "分钟价差截面",   R"(z(\mathrm{spread})_{\mathrm{cs}})",                     "分钟spread截面z-score")\
  X(next_1m_ret,        1, DATA, LB,   LABEL,      FUTURE_RET, NONE,        "Next 1-Minute Return",        "下1分钟收益",    R"(\log\frac{c_{t+1}}{c_t})",                          "下一分钟对数收益")\
  X(calmar_score,       1, DATA, LB,   LABEL,      SCORE,      NONE,        "Calmar Score",                "Calmar评分",     R"(\frac{\mathrm{ret}}{\mathrm{maxDD}})",                 "年化收益/最大回撤")\
  X(universe_size,      1, DATA, SH,   META,       UNIVERSE,   NONE,        "Universe Size",               "全域规模",       R"(\#(\mathrm{valid}))",                                 "当前有效合约数量")\
  X(market_return,      1, DATA, SH,   META,       BENCHMARK,  NONE,        "Market Return",               "市场收益",       R"(\log\frac{\mathrm{mkt}_t}{\mathrm{mkt}_{t-1}})",      "市场基准收益率")\
  X(_ohlc_open,         1, DATA, META, META,       RAW,        NONE,        "OHLC Open",                   "开盘价",         R"(O)",                                                  "GUI:分钟开盘价(分)")\
  X(_ohlc_high,         1, DATA, META, META,       RAW,        NONE,        "OHLC High",                   "最高价",         R"(H)",                                                  "GUI:分钟最高价(分)")\
  X(_ohlc_low,          1, DATA, META, META,       RAW,        NONE,        "OHLC Low",                    "最低价",         R"(L)",                                                  "GUI:分钟最低价(分)")\
  X(_ohlc_close,        1, DATA, META, META,       RAW,        NONE,        "OHLC Close",                  "收盘价",         R"(C)",                                                  "GUI:分钟收盘价(分)")\
  X(_ohlc_volume,       1, DATA, META, META,       RAW,        NONE,        "OHLC Volume",                 "成交量",         R"(V)",                                                  "GUI:分钟成交量")\
  X(_data_valid,        1, ALL,  META, META,       RAW,        NONE,        "Data Valid Flag",             "数据有效标志",   R"(\mathbf{1}_{\mathrm{valid}})",                        "事件驱动稀疏性标记")
...
#define LEVEL_2_FIELDS(X)\
  X(hour_ret_12h_mom,    1, DATA, TS,   MOMENTUM,   NORMALIZED, ZSCORE,      "Hour Return 12h Momentum",    "12小时动量",     R"(\frac{\sum_{12h} r}{z})",                              "12小时动量标准化")\
  X(hour_volatility,     1, DATA, TS,   VOLATILITY, NORMALIZED, LOG_ZSCORE,  "Hour Volatility 24h",         "24小时波动率",   R"(\log(\sigma_{24h}))",                                  "24小时波动率标准化")\
  X(pivot_dev,           1, DATA, TS,   PRICE,      DEVIATION,  CLIP,        "Pivot Deviation",             "Pivot偏差",      R"(\frac{c - \mathrm{pivot}}{\mathrm{range}})",              "收盘相对pivot偏差")\
  X(dominant_persist,    1, DATA, TS,   IMBALANCE,  OSCILLATOR, ZSCORE,      "Dominant Persistence",        "主导持续性",     R"(\mathrm{EMA}(\mathrm{side}))",                             "买卖主导延续性")\
  X(hour_overnight_gap,  1, DATA, TS,   PRICE,      DEVIATION,  WINSOR,      "Hour Overnight Gap",          "隔夜跳空",       R"(\frac{o - c_{-1}}{\sigma})",                           "隔夜gap捕捉消息冲击")\
  X(cs_hour_return_beta, 1, DATA, CS,   MOMENTUM,   RANK,       RANK_ZSCORE, "CS Hour Return Beta",         "小时收益残差",   R"(\epsilon_{r \sim \mathrm{mkt}})",                       "相对市场回归残差排名")\
  X(cs_hour_liq_adj_ret, 1, DATA, CS,   MOMENTUM,   RANK,       RANK_ZSCORE, "CS Hour Liq Adj Return",      "流动性调整收益", R"(\frac{r}{\sqrt{\mathrm{vol}}})",                        "流动性调整后收益排名")\
  X(cs_hour_range_rank,  1, DATA, CS,   VOLATILITY, RANK,       RANK_ZSCORE, "CS Hour Range Rank",          "小时Range排名",  R"(\Phi^{-1}(\mathrm{pctl}(\mathrm{range})))",               "价格区间截面排名")\
  X(next_1h_ret,         1, DATA, LB,   LABEL,      FUTURE_RET, NONE,        "Next 1-Hour Return",          "下1小时收益",    R"(\log\frac{c_{t+1h}}{c_t})",                            "下一小时对数收益")\
  X(sharpe_score,        1, DATA, LB,   LABEL,      SCORE,      NONE,        "Sharpe Score",                "Sharpe评分",     R"(\frac{\mu - r_f}{\sigma})",                           "超额收益/波动率")\
  X(universe_size,       1, DATA, SH,   META,       UNIVERSE,   NONE,        "Universe Size",               "全域规模",       R"(\#(\mathrm{valid}))",                                    "当前有效合约数量")\
  X(market_volatility,   1, DATA, SH,   META,       BENCHMARK,  NONE,        "Market Volatility",           "市场波动率",     R"(\mathrm{std}(r_{\mathrm{mkt},24h}))",                      "市场24小时波动率")\
  X(_data_valid,         1, ALL,  META, META,       RAW,        NONE,        "Data Valid Flag",             "数据有效标志",   R"(\mathbf{1}_{\mathrm{valid}})",                           "事件驱动稀疏性标记")
...
#define DEPTH_FIELDS(X)                                                                                                                                                        \
  X(_bid_price,    L2::LOB_DEPTH, DEPTH, META, META, RAW, NONE, "Bid Prices",        "买盘价格", R"(P^B_{0:N})",               "GUI:N档买盘价格(分)")      \
  X(_ask_price,    L2::LOB_DEPTH, DEPTH, META, META, RAW, NONE, "Ask Prices",        "卖盘价格", R"(P^A_{0:N})",               "GUI:N档卖盘价格(分)")      \
  X(_bid_volume,   L2::LOB_DEPTH, DEPTH, META, META, RAW, NONE, "Bid Volumes",       "买盘量",   R"(V^B_{0:N})",               "GUI:N档买盘量(手,100股)")  \
  X(_ask_volume,   L2::LOB_DEPTH, DEPTH, META, META, RAW, NONE, "Ask Volumes",       "卖盘量",   R"(V^A_{0:N})",               "GUI:N档卖盘量(手,100股)")  \
  X(_mid_price,    1,             DEPTH, META, META, RAW, NONE, "Mid Price",         "中间价",   R"(\frac{P^B_1 + P^A_1}{2})",  "GUI:实时中间价(分)")       \
  X(_depth_valid,  1,             ALL,   META, META, RAW, NONE, "Depth Valid Flag",  "深度有效", R"(\mathbf{1}_{\mathrm{valid}})",  "LOB深度缓冲区完整性标记")  \
  X(_data_valid,   1,             ALL,   META, META, RAW, NONE, "Data Valid Flag",   "数据有效", R"(\mathbf{1}_{\mathrm{valid}})",  "事件驱动稀疏性标记")
