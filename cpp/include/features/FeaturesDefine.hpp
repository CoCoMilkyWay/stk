...
// clang-format off
// ============================================================================
// 特征表 = 两张表, 都在本文件, 是用户改特征时唯一要碰的地方 (算子数学在 Operator/TS/**.hpp):
//
//   NODES  节点表: 一行 = 一个算子实例 = 计算图里的一个节点 (什么时候算, 输入是谁)
//   LEVEL_*_FIELDS 字段表: 一行 = 一个落盘列 (元数据 + SRC: 这一列的值从哪来)
//
// 基建 (ComputeGraph / Tick_Sequential / Minute_Sequential / *_Crosssection / FeatureStore /
// Feature.hpp GUI 元数据) 全部由这两张表展开生成, 不需要手改.
//
// 加一个普通特征 = 算子文件 + NODES 一行 + FIELDS 一行 (新算子文件另需在 ComputeGraph.hpp 加 #include).
// 给已有算子加输出口 = 算子里加一个 Out 枚举 + FIELDS 一行.  删特征 = 删对应行.
...
// NODES: 计算图节点表
// ============================================================================
// Format: N(name, (OpType), (inputs...), compute_trigger, flush_trigger)
//   name     节点名 (DAG 成员名; 字段表 OP(name, out) / 下游输入 name.out(...) 都用它)
...
//
// 行序 = 声明序 = 同一触发域内的执行序 (拓扑序由人保证: 上游必须排在下游前面).
// 各触发域一个 tick 内的执行顺序: onTaker|onMaker|onCancel → onTick → onDepth; 分钟边界 → onMinute.
// 节点的输出口由算子的 enum Out 定义, 输出 CBuffer 由 Node 持有 (算子只拿引用).
// 标签 (LabelReturn*) 回填别的时间行, 不走本表, 在 ComputeGraph.hpp 手工声明.

#define NODES(N)\
  /* ---- 源层: 订单 / 时间索引 / 盘口 ---- */\
  N(Taker,          (OrderInfo),            (tick_data),                                                          onTaker,  onTaker )\
  N(Maker,          (OrderInfo),            (tick_data),                                                          onMaker,  onMaker )\
  N(Cancel,         (OrderInfo),            (tick_data),                                                          onCancel, onCancel)\
  N(TickIndex,      (TickIndex),            (tick_data),                                                          onTick,   onTick  )\
  N(DepthIndex,     (DepthIndex),           (tick_data),                                                          onDepth,  onDepth )\
  N(DepthData,      (DepthData<L2::LOB_DEPTH>), (tick_data, Taker.out(Taker.price), asset_code_),                 onDepth,  onDepth )\
  /* ---- L0 采样 (onDepth) ---- */\
  N(MidPrice,       (MidPrice),             (DepthData.bid_price[0], DepthData.ask_price[0]),                     onDepth,  onDepth )\
  N(MicroPrice,     (MicroPrice),           (DepthData.bid_price[0], DepthData.ask_price[0], DepthData.bid_qty[0], DepthData.ask_qty[0]), onDepth, onDepth)\
  N(Spread,         (Spread),               (DepthData.bid_price[0], DepthData.ask_price[0]),                     onDepth,  onDepth )\
  N(Ci_1,           (CI<1>),                (DepthData.bid_qty, DepthData.ask_qty),                               onDepth,  onDepth )\
  N(Ofi_1,          (OFI<1>),               (DepthData.bid_qty, DepthData.ask_qty, DepthData.bid_price, DepthData.ask_price), onDepth, onDepth)\
  N(Ofi_5,          (OFI<5>),               (DepthData.bid_qty, DepthData.ask_qty, DepthData.bid_price, DepthData.ask_price), onDepth, onDepth)\
  /* ---- L1 采样 (onMinute) ---- */\
  N(MinuteIndex,    (MinuteIndex),          (minute_data),                                                        onMinute, onMinute)\
  N(Ci_5,           (CI<5>),                (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(Ci_10,          (CI<10>),               (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(Ci_30,          (CI<30>),               (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(Ci_all,         (CI_all),               (tick_data),                                                          onMinute, onMinute)\
  N(Cwi_1,          (CWI<10>),              (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(Cwi_2,          (CWI<20>),              (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(Ddi_1,          (DDI<1>),               (DepthData.bid_qty, DepthData.ask_qty, DepthData.bid_price, DepthData.ask_price), onMinute, onMinute)\
  N(Ddi_2,          (DDI<2>),               (DepthData.bid_qty, DepthData.ask_qty, DepthData.bid_price, DepthData.ask_price), onMinute, onMinute)\
  N(Tbr_5,          (TLR<5, true>),         (DepthData.bid_qty, DepthData.ask_qty, tick_data),                    onMinute, onMinute)\
  N(Tar_5,          (TLR<5, false>),        (DepthData.bid_qty, DepthData.ask_qty, tick_data),                    onMinute, onMinute)\
  N(Para_b_c0,      (Para<true, 0>),        (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(Para_b_c1,      (Para<true, 1>),        (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(Para_b_c2,      (Para<true, 2>),        (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(Para_a_c0,      (Para<false, 0>),       (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(Para_a_c1,      (Para<false, 1>),       (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(Para_a_c2,      (Para<false, 2>),       (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(ParaImba_c0,    (ParaImba<0>),          (Para_b_c0.out(), Para_a_c0.out()),                                   onMinute, onMinute)\
  N(ParaImba_c1,    (ParaImba<1>),          (Para_b_c1.out(), Para_a_c1.out()),                                   onMinute, onMinute)\
  N(ParaImba_c2,    (ParaImba<2>),          (Para_b_c2.out(), Para_a_c2.out()),                                   onMinute, onMinute)\
  N(Grad_b_5_c1,    (Grad<5, true>),        (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(Grad_a_5_c1,    (Grad<5, false>),       (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(GradImba_5_c1,  (GradImba),             (Grad_b_5_c1.out(), Grad_a_5_c1.out()),                               onMinute, onMinute)\
  N(Entropy_b_5,    (Entropy<5, true>),     (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(Entropy_a_5,    (Entropy<5, false>),    (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(Entropy_b_30,   (Entropy<30, true>),    (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(Entropy_a_30,   (Entropy<30, false>),   (DepthData.bid_qty, DepthData.ask_qty),                               onMinute, onMinute)\
  N(EntropyImba_5,  (EntropyImba),          (Entropy_b_5.out(), Entropy_a_5.out()),                               onMinute, onMinute)\
  N(EntropyImba_30, (EntropyImba),          (Entropy_b_30.out(), Entropy_a_30.out()),                             onMinute, onMinute)\
  N(DepthRepresentation, (DepthRepresentation), (),                                                              onMinute, onMinute)\
  /* ---- L1 降频: 逐笔累计 (onTick), 分钟输出 (onMinute) ---- */\
  N(Ctr,            (CTR),                  (tick_data),                                                          onTick,   onMinute)\
  N(Oa,             (OA),                   (tick_data),                                                          onTick,   onMinute)\
  N(Hla,            (HLA),                  (tick_data, DepthData.bid_qty, DepthData.ask_qty),                    onTick,   onMinute)\
  N(ToxicCr,        (ToxicCr),              (tick_data),                                                          onTick,   onMinute)\
  N(FlowRate,       (FlowRate),             (tick_data),                                                          onTick,   onMinute)\
  N(Behav,          (Behav),                (tick_data),                                                          onTick,   onMinute)\
  N(Manip,          (Manip),                (tick_data, DepthData.bid_qty, DepthData.ask_qty),                    onTick,   onMinute)\
  N(Resiliency,     (Resiliency),           (tick_data, DepthData.bid_qty, DepthData.ask_qty),                    onTick,   onMinute)\
  /* ---- L1 采样 (onMinute, 续) ---- */\
  N(Cost_buy_1,     (Cost<1, true>),        (DepthData.ask_price, DepthData.ask_qty, MidPrice.out()),             onMinute, onMinute)\
  N(Cost_buy_5,     (Cost<5, true>),        (DepthData.ask_price, DepthData.ask_qty, MidPrice.out()),             onMinute, onMinute)\
  N(Cost_buy_10,    (Cost<10, true>),       (DepthData.ask_price, DepthData.ask_qty, MidPrice.out()),             onMinute, onMinute)\
  N(Cost_sell_1,    (Cost<1, false>),       (DepthData.bid_price, DepthData.bid_qty, MidPrice.out()),             onMinute, onMinute)\
  N(Cost_sell_5,    (Cost<5, false>),       (DepthData.bid_price, DepthData.bid_qty, MidPrice.out()),             onMinute, onMinute)\
  N(Cost_sell_10,   (Cost<10, false>),      (DepthData.bid_price, DepthData.bid_qty, MidPrice.out()),             onMinute, onMinute)\
  N(Peak_loc_bid,   (Peak<true, true>),     (DepthData.bid_qty),                                                  onMinute, onMinute)\
  N(Peak_loc_ask,   (Peak<false, true>),    (DepthData.ask_qty),                                                  onMinute, onMinute)\
  N(Peak_ratio_bid, (Peak<true, false>),    (DepthData.bid_qty),                                                  onMinute, onMinute)\
  N(Peak_ratio_ask, (Peak<false, false>),   (DepthData.ask_qty),                                                  onMinute, onMinute)\
  N(Val,            (Valuation),            (minute_data, fund_row_),                                             onMinute, onMinute)

// ============================================================================
// FIELDS: 落盘字段表
// ============================================================================
// Format: X(code, width, valid_type, data_type, cat_l1, cat_l2, norm_method, PSD, name_en, name_cn, description, formula, SRC)
//
// SRC (这一列的值从哪来; 基建按它生成写回代码 / 截面配置 / 广播):
//   OP(Node)              单输出节点的输出口 (Node::Out::value)
//   OP(Node, port)        多输出节点的某个口 (Node::Out::port)
//   FUND(field)           当日基本面行 fund::field 盘中广播 (仅 L1)
//   CS(lvl, src, tf, m)   截面: 源层 lvl (0/1) 的字段 src → cs::Transform::tf → cs::Method::m
//   META                  基建自己写 (有效标志 / 索引 / OHLC)
//   LABEL                 标签回填 (Tick_Sequential 手工写)
// 写回时机: L0 的 OP 列每 tick 末写 (值 = 输出口最新值, 尚无输出时为 0); L1 的 OP/FUND 列每有效分钟写.
// SRC 与 data_type 的对应由 FeatureStoreConfig 静态断言: OP/FUND→TS, CS→CS, LABEL→LB, META→META.
...
#define LEVEL_0_FIELDS(X)\
  /* Basic (基础特征) */\
  X(sec,                1, DATA,  TS,   BASIC,      OSCILLATOR, SINCOS,      "100/00/00", "Time Sec Phase",                   "时间-秒相位",           "用于因子组合",                                         R"(\sin(\frac{2\pi t}{60\mathrm{s}}))", OP(TickIndex, sec))\
  ...
  /* ======== 元数据 (Metadata) ======== */\
  X(_link_to_L1,        1, ALL,   META, META,       RAW,        NONE,        "100/00/00", "Link to L1",                       "L1时间索引",            "L0→L1时间映射",                                        R"(\mathrm{idx}_{L1})", META)\
  X(_depth_valid,       1, ALL,   META, META,       RAW,        NONE,        "100/00/00", "Depth Valid Flag",                 "深度有效标志",          "LOB深度缓冲区完整性标记",                              R"(\mathbf{1}_{\mathrm{valid}})", META)\
  X(_data_valid,        1, ALL,   META, META,       RAW,        NONE,        "100/00/00", "Data Valid Flag",                  "数据有效标志",          "事件驱动稀疏性标记",                                   R"(\mathbf{1}_{\mathrm{valid}})", META)\
...
#define LEVEL_1_FIELDS(X)\
  ...
  X(imba_5_c1,          1, DATA,  TS,   IMBALANCE,  RATIO,      NONE,        "00/100/00", "Top 5-level Grad Ratio",           "买卖五档梯度失衡",      "买卖五档梯度失衡(降频)",                               R"(\frac{|\sum_{i=1}^{N-1}(V_{i+1,t}^{M,B} - V_{i,t}^{M,B})| - |\sum_{i=1}^{N-1}(V_{i+1,t}^{M,A} - V_{i,t}^{M,A})|}{|\sum_{i=1}^{N-1}(V_{i+1,t}^{M,B} - V_{i,t}^{M,B})| + |\sum_{i=1}^{N-1}(V_{i+1,t}^{M,A} - V_{i,t}^{M,A})|}, \quad N = 5)", OP(GradImba_5_c1))\
  ...
  X(cc_r,               1, DATA,  TS,   ORDER_FLOW, RATIO,      NONE,        "00/100/00", "CC Trade Ratio",                   "连续竞价成交占比",      "连续竞价成交额占全天成交额的比例(降频)",               R"(\frac{|O_t^{T,\mathrm{CA}}|}{|O_t^{T}|})", OP(Ctr, cc_r))\
  X(ctr_xl,             1, DATA,  TS,   ORDER_FLOW, RATIO,      NONE,        "00/100/00", "Cumulative XL Trade Ratio",        "特大单累计成交占比",    "从开盘到t时刻,特大单成交额占总成交额比例(降频)",       R"(\frac{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B,\mathrm{XL}}|+|O_{\tau}^{T,A,\mathrm{XL}}|)}{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B}|+|O_{\tau}^{T,A}|)})", OP(Ctr, ctr_xl))\
  X(ctr_l,              1, DATA,  TS,   ORDER_FLOW, RATIO,      NONE,        "00/100/00", "Cumulative L Trade Ratio",         "大单累计成交占比",      "从开盘到t时刻,大单成交额占总成交额比例(降频)",         R"(\frac{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B,\mathrm{L}}|+|O_{\tau}^{T,A,\mathrm{L}}|)}{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B}|+|O_{\tau}^{T,A}|)})", OP(Ctr, ctr_l))\
  X(ctr_m,              1, DATA,  TS,   ORDER_FLOW, RATIO,      NONE,        "00/100/00", "Cumulative M Trade Ratio",         "中单累计成交占比",      "从开盘到t时刻,中单成交额占总成交额比例(降频)",         R"(\frac{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B,\mathrm{M}}|+|O_{\tau}^{T,A,\mathrm{M}}|)}{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B}|+|O_{\tau}^{T,A}|)})", OP(Ctr, ctr_m))\
  X(ctr_s,              1, DATA,  TS,   ORDER_FLOW, RATIO,      NONE,        "00/100/00", "Cumulative S Trade Ratio",         "小单累计成交占比",      "从开盘到t时刻,小单成交额占总成交额比例(降频)",         R"(\frac{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B,\mathrm{S}}|+|O_{\tau}^{T,A,\mathrm{S}}|)}{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B}|+|O_{\tau}^{T,A}|)})", OP(Ctr, ctr_s))\
  X(cnbi,               1, DATA,  TS,   ORDER_FLOW, RATIO,      NONE,        "00/100/00", "Cumulative Net Buy Ratio",         "累计净买入比率",        "从开盘到t,主动买入成交额减主动卖出成交额(降频)",       R"(\frac{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B}|-|O_{\tau}^{T,A}|)}{\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B}|+|O_{\tau}^{T,A}|)})", OP(Ctr, cnbi))\
  X(cnbi_xl,            1, DATA,  TS,   ORDER_FLOW, RATIO,      NONE,        "00/100/00", "Cumulative Net Buy Ratio XL",      "特大单累计净买入比率",  "特大单对累计净买入失衡的贡献(降频)",                   R"(\frac{N_t^{\mathrm{XL}}}{\sum_{c}|N_t^{c}|}, \quad N_t^{c}=\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B,c}|-|O_{\tau}^{T,A,c}|), \quad c\in\{\mathrm{XL,L,M,S}\})", OP(Ctr, cnbi_xl))\
  X(cnbi_l,             1, DATA,  TS,   ORDER_FLOW, RATIO,      NONE,        "00/100/00", "Cumulative Net Buy Ratio L",       "大单累计净买入比率",    "大单对累计净买入失衡的贡献(降频)",                     R"(\frac{N_t^{\mathrm{L}}}{\sum_{c}|N_t^{c}|}, \quad N_t^{c}=\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B,c}|-|O_{\tau}^{T,A,c}|), \quad c\in\{\mathrm{XL,L,M,S}\})", OP(Ctr, cnbi_l))\
  X(cnbi_m,             1, DATA,  TS,   ORDER_FLOW, RATIO,      NONE,        "00/100/00", "Cumulative Net Buy Ratio M",       "中单累计净买入比率",    "中单对累计净买入失衡的贡献(降频)",                     R"(\frac{N_t^{\mathrm{M}}}{\sum_{c}|N_t^{c}|}, \quad N_t^{c}=\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B,c}|-|O_{\tau}^{T,A,c}|), \quad c\in\{\mathrm{XL,L,M,S}\})", OP(Ctr, cnbi_m))\
  X(cnbi_s,             1, DATA,  TS,   ORDER_FLOW, RATIO,      NONE,        "00/100/00", "Cumulative Net Buy Ratio S",       "小单累计净买入比率",    "小单对累计净买入失衡的贡献(降频)",                     R"(\frac{N_t^{\mathrm{S}}}{\sum_{c}|N_t^{c}|}, \quad N_t^{c}=\sum_{\tau=t_0}^{t}(|O_{\tau}^{T,B,c}|-|O_{\tau}^{T,A,c}|), \quad c\in\{\mathrm{XL,L,M,S}\})", OP(Ctr, cnbi_s))\
  X(cnbi_am,            1, DATA,  TS,   ORDER_FLOW, RATIO,      NONE,        "00/100/00", "Net Buy Ratio AM",                 "早盘净买入比率",        "上午有符号净主动成交额(降频)",                         R"(\frac{\sum_{\tau\in\mathcal{T}_{\mathrm{AM}}}(|O_{\tau}^{T,B}|-|O_{\tau}^{T,A}|)}{\sum_{\tau\in\mathcal{T}_{\mathrm{AM}}}(|O_{\tau}^{T,B}|+|O_{\tau}^{T,A}|)})", OP(Ctr, cnbi_am))\
  X(cnbi_pm,            1, DATA,  TS,   ORDER_FLOW, RATIO,      NONE,        "00/100/00", "Net Buy Ratio PM",                 "尾盘净买入比率",        "下午有符号净主动成交额(降频)",                         R"(\frac{\sum_{\tau\in\mathcal{T}_{\mathrm{PM}}}(|O_{\tau}^{T,B}|-|O_{\tau}^{T,A}|)}{\sum_{\tau\in\mathcal{T}_{\mathrm{PM}}}(|O_{\tau}^{T,B}|+|O_{\tau}^{T,A}|)})", OP(Ctr, cnbi_pm))\
  X(oa_bcr,             1, DATA,  TS,   ORDER_FLOW, RATIO,      NONE,        "00/100/00", "OA Bid Cancel Ratio",              "早盘竞价买方撤单率",    "集合竞价买方撤单额占买方挂单额比例(降频)",             R"(\frac{\sum_{\tau\in\mathcal{T}_{\mathrm{OA}}}|O_{\tau}^{C,B}|}{\sum_{\tau\in\mathcal{T}_{\mathrm{OA}}}|O_{\tau}^{M,B}|}, \quad \mathcal{T}_{\mathrm{OA}}=[09\!:\!15,09\!:\!25))", OP(Oa, bcr))\
  X(oa_acr,             1, DATA,  TS,   ORDER_FLOW, RATIO,      NONE,        "00/100/00", "OA Ask Cancel Ratio",              "早盘竞价卖方撤单率",    "集合竞价卖方撤单额占卖方挂单额比例(降频)",             R"(\frac{\sum_{\tau\in\mathcal{T}_{\mathrm{OA}}}|O_{\tau}^{C,A}|}{\sum_{\tau\in\mathcal{T}_{\mathrm{OA}}}|O_{\tau}^{M,A}|}, \quad \mathcal{T}_{\mathrm{OA}}=[09\!:\!15,09\!:\!25))", OP(Oa, acr))\
  X(oa_btr,             1, DATA,  TS,   ORDER_FLOW, RATIO,      NONE,        "00/100/00", "OA Bid Trade Ratio",               "早盘竞价买方成交率",    "集合竞价买方成交额占买方挂单额比例(降频)",             R"(\frac{\sum_{\tau\in\mathcal{T}_{\mathrm{OA}}}|O_{\tau}^{T,B}|}{\sum_{\tau\in\mathcal{T}_{\mathrm{OA}}}|O_{\tau}^{M,B}|}, \quad \mathcal{T}_{\mathrm{OA}}=[09\!:\!15,09\!:\!25))", OP(Oa, btr))\
  X(oa_atr,             1, DATA,  TS,   ORDER_FLOW, RATIO,      NONE,        "00/100/00", "OA Ask Trade Ratio",               "早盘竞价卖方成交率",    "集合竞价卖方成交额占卖方挂单额比例(降频)",             R"(\frac{\sum_{\tau\in\mathcal{T}_{\mathrm{OA}}}|O_{\tau}^{T,A}|}{\sum_{\tau\in\mathcal{T}_{\mathrm{OA}}}|O_{\tau}^{M,A}|}, \quad \mathcal{T}_{\mathrm{OA}}=[09\!:\!15,09\!:\!25))", OP(Oa, atr))\
  ...
  /* ======== 截面因子 (qmt 忠实移植, 每分钟; 方法 = SRC 列 CS(源层, 源列, 变换, 方法); 中性化=行业+log市值 OLS 残差) ======== */\
  ...
  X(_ohlc_open,         1, DATA,  META, META,       RAW,        NONE,        "00/100/00", "OHLC Open",                        "开盘价",                "GUI:分钟开盘价(分)",                                   R"(O)", META)\
  X(_ohlc_high,         1, DATA,  META, META,       RAW,        NONE,        "00/100/00", "OHLC High",                        "最高价",                "GUI:分钟最高价(分)",                                   R"(H)", META)\
  X(_ohlc_low,          1, DATA,  META, META,       RAW,        NONE,        "00/100/00", "OHLC Low",                         "最低价",                "GUI:分钟最低价(分)",                                   R"(L)", META)\
  X(_ohlc_close,        1, DATA,  META, META,       RAW,        NONE,        "00/100/00", "OHLC Close",                       "收盘价",                "GUI:分钟收盘价(分)",                                   R"(C)", META)\
  X(_ohlc_volume,       1, DATA,  META, META,       RAW,        NONE,        "00/100/00", "OHLC Volume",                      "成交量",                "GUI:分钟成交量",                                       R"(V)", META)\
  X(_data_valid,        1, ALL,   META, META,       RAW,        NONE,        "00/100/00", "Data Valid Flag",                  "数据有效标志",          "事件驱动稀疏性标记",                                   R"(\mathbf{1}_{\mathrm{valid}})", META)

// ============================================================================
// DEPTH: LOB Snapshot Data (separate storage for orderflow visualization)
// ============================================================================
// Format 同 FIELDS; 全部 META (Tick_Sequential 手工写盘口快照)

#define DEPTH_FIELDS(X)\
  X(_bid_price,         L2::LOB_DEPTH, DEPTH, META, META, RAW, NONE,         "00/00/00",  "Bid Prices",        "买盘价格", "GUI:N档买盘价格(分)",       R"(P^{M,B}_{0:N})", META)\
  X(_ask_price,         L2::LOB_DEPTH, DEPTH, META, META, RAW, NONE,         "00/00/00",  "Ask Prices",        "卖盘价格", "GUI:N档卖盘价格(分)",       R"(P^{M,A}_{0:N})", META)\
  X(_bid_volume,        L2::LOB_DEPTH, DEPTH, META, META, RAW, NONE,         "00/00/00",  "Bid Volumes",       "买盘量",   "GUI:N档买盘量(手,100股)",   R"(V^{M,B}_{0:N})", META)\
  X(_ask_volume,        L2::LOB_DEPTH, DEPTH, META, META, RAW, NONE,         "00/00/00",  "Ask Volumes",       "卖盘量",   "GUI:N档卖盘量(手,100股)",   R"(V^{M,A}_{0:N})", META)\
  X(_mid_price,         1,             DEPTH, META, META, RAW, NONE,         "00/00/00",  "Mid Price",         "中间价",   "GUI:实时中间价(分)",        R"(\frac{P^{M,B}_1 + P^{M,A}_1}{2})", META)\
  X(_depth_valid,       1,             ALL,   META, META, RAW, NONE,         "00/00/00",  "Depth Valid Flag",  "深度有效", "LOB深度缓冲区完整性标记",   R"(\mathbf{1}_{\mathrm{valid}_{depth}})", META)\
  X(_data_valid,        1,             ALL,   META, META, RAW, NONE,         "00/00/00",  "Data Valid Flag",   "数据有效", "事件驱动稀疏性标记",        R"(\mathbf{1}_{\mathrm{valid}_{data}})", META) // clang-format on
