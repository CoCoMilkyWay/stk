...
struct Dag {
  ...
  std::vector<std::vector<int>> free_after; // free_after[i] = 节点 i 执行完可释放的槽
  int n_slots = 0;
  int n_ops = 0;
...
struct DagBuilder {
  const expr::Expr &e;
  Dag &d;
  std::map<std::string, int> memo;     // 子树规范串 → 节点下标
...
inline void plan(Dag &d) {
  ...
  // 根的槽不在任何 free_after 里 (无消费者), 留给调用方取结果
}

inline Dag build(const expr::Expr &e) {
  assert(!e.nodes.empty());
  Dag d;
  detail::DagBuilder b{e, d, {}, {}};
  b.visit(0);
  plan(d);
  return d;
}
