...
// ---- SELF × 有窗 (50 = Cum 23 + Roll 26 + Ema 1); 按统计核排, 同核的不同窗相邻 ----
#define OP_TS_WIN(X)                                                                                                                                                                  \
...
// ---- POINT × {ALL, GROUP} (20): 截面, T 窗恒 POINT (只看当前时刻) ----
#define OP_CS(X)                                                                                                                                                                                                \
...
// ---- 全表 (行格式统一, 消费者可用 OP_ALL 一次展开; 需要按 A 域分派时用 X_##域 token 粘贴) ----
#define OP_TS(X) OP_TS_POINT(X) OP_TS_WIN(X)
#define OP_ALL(X) OP_TS(X) OP_CS(X)
